...
const Carousel = ({ letter = "?", image, setIndex, defaultColor }) => {
	...
	const onMomentumScrollEnd = (event) => {
		...
		if (index >= 0 && index < colors.length) {
			...
			if (setIndex) setIndex(colors[index].id);
		}
	};
	...
	const renderItem = ({ item, index }) => {
		...
		return (
			...
			>
				...
				>
					{image ? <Image source={image} style={styles.image} /> : <Text style={styles.circleText}>{letter}</Text>}
				</Animated.View>
			</View>
		);
	};
	...
	return (
		<View style={styles.container}>
			<Animated.FlatList
				ref={flatListRef}
				horizontal
				data={[{ key: "spacer-left" }, ...colors, { key: "spacer-right" }]}
