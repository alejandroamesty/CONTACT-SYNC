...
const ContactDetail = ({ route, navigation: { goBack } }) => {
                    ...
                    response
                        ...
                        .then((fetchedContact) => {
                            ...
                            if (contact) {
                                ...
                                const formattedPhones = contact.phones.map(
                                    (phone) => ({
                                        type:
                                            phoneMapping[phone.phone_type] ||
                                            "home",
                                ...
                                const formattedEmails = contact.emails.map(
                                    (email) => ({
                                        typeLabel:
                                            emailMapping[email.email_type] ||
                                            "home",
                                ...
                                const formattedDates = contact.dates.map(
                                    (date) => ({
                                        typeLabel:
                                            dateMapping[date.date_type] ||
                                            "birthday",
                                ...
                                const formattedUrls = contact.urls.map(
                                    (url) => ({
                                        typeLabel:
                                            urlMapping[url.url_type] ||
                                            "website",
                                        type: url.url_type || 1,
                                        url: url.url || "",
                                    })
    ...
    const addPhoneNumber = () => {
        setPhoneNumbers([
            ...phoneNumbers,
            { type: "home", phoneType: 1, phoneCode: "", phoneNumber: "" },
        ]);
    };
    ...
    const addDate = () => {
        setDates([
            ...dates,
            { typeLabel: "birthday", type: 1, date: new Date() },
        ]);
    };
            ...
            <ScrollView style={styles.container} indicatorStyle="white">
                <View style={styles.infoContainer}>
                    ...
                    {displayURLs.map((url, index) => (
                        <InfoContainer
                            key={index}
                            name={url ? urlMapping[url.url] : "URL"}
                            info={url.url}
                            style={{ marginBottom: 10 }}
                        />
                    ))}
